...
-- Excel live links and memo pack tables for Phase 5
...
create table if not exists excel_connections (
  ...
  target_json jsonb not null,  -- { baseline_id, baseline_version, run_id?, draft_session_id? }
  workbook_json jsonb,         -- { file_name, sheet_names }
  bindings_json jsonb not null default '[]'::jsonb,  -- array of binding objects
  sync_json jsonb,             -- { refresh_interval_seconds, auto_pull, push_behavior }
  permissions_json jsonb,      -- { can_push_roles: ["owner", "admin", "analyst"] }
...
create table if not exists excel_sync_events (
  ...
  target_json jsonb,           -- { baseline_id, run_id, changeset_id }
  diff_json jsonb not null,    -- { changed: [...], errors: [...] }
  primary key (tenant_id, event_id),
  foreign key (tenant_id, excel_connection_id)
    references excel_connections(tenant_id, excel_connection_id)
    on delete cascade
);
create index if not exists idx_sync_events_connection on excel_sync_events(tenant_id, excel_connection_id);
create index if not exists idx_sync_events_timestamp on excel_sync_events(tenant_id, timestamp);
...
create table if not exists memo_packs (
  ...
  title text,
  source_json jsonb not null,  -- { baseline_id, baseline_version, run_id, scenario_id? }
  sections_json jsonb not null,  -- array of section objects with content blocks
  outputs_json jsonb not null,   -- { html_path, pdf_path?, docx_path? }
